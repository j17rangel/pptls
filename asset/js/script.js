...
vidasJugador = 3, 
vidasRival = 3,
personajes = ["sheldon","leonard","rajesh","howard","penny","bernadette"],
idAtaques = ["selec-piedra","selec-papel","selec-tijera","selec-lagarto","selec-spock"],
...
function iniciarJuego(){
    ...
    botonReglasAbrir.addEventListener('mouseover' || 'click', () => {
            ocultarMostrar('div-reglas-juego', 'flex');
        })
    let botonReglasCerrar = document.getElementById('boton-reglas');
    botonReglasCerrar.addEventListener('mouseout', () => {
...
function seleccionJugador(botonDesabilitar){
    ...
    function selecAtaqueJugador(idBoton, nombreAtaque, desabilitarBoton){
        ...
        if(desabilitarBoton){
            ...
            let imagenAtaqueJugador = document.querySelector(`button[id='${idBoton}'] img`).src;
            document.querySelector("div[id='vs-ataque-jugador'] img").src = imagenAtaqueJugador;
...
function selecPersonajeJugador(){
    let jugador = document.querySelector("#elegir-personaje input[type='radio']:checked");
    if(jugador){
        ...
        let imagenJugador = document.querySelector(`label[for='${nombreJugador}'] img`).src;
        ...
        document.getElementById("vida-jugador").innerHTML = '<i class="fi fi-ss-heart"></i><i class="fi fi-ss-heart"></i><i class="fi fi-ss-heart"></i>';
...
function combate(){
    let vidasCorazon = ['-','<i class="fi fi-ss-heart"></i>','<i class="fi fi-ss-heart"></i><i class="fi fi-ss-heart"></i>']
    if(ataqueEnemigo == ataqueJugador){
        mensajes('EMPATE','');
    } else if ((ataqueEnemigo == "PIEDRA" && ataqueJugador == "PAPEL") || (ataqueEnemigo == "PAPEL" && ataqueJugador == "TIJERA") || (ataqueEnemigo == "TIJERA" && ataqueJugador == "PIEDRA") || (ataqueEnemigo == "LAGARTO" && ataqueJugador == "PIEDRA") || (ataqueEnemigo == "SPOCK" && ataqueJugador == "LAGARTO") || (ataqueEnemigo == "TIJERA" && ataqueJugador == "SPOCK") || (ataqueEnemigo == "LAGARTO" && ataqueJugador == "TIJERA") || (ataqueEnemigo == "PAPEL" && ataqueJugador == "LAGARTO")|| (ataqueEnemigo == "SPOCK" && ataqueJugador == "PAPEL") || (ataqueEnemigo == "PIEDRA" && ataqueJugador == "SPOCK")){
        mensajes('Ganaste','Rival -<i class="fi fi-ss-heart" id="icon-mensaje"></i>');
        document.getElementById("vida-rival").innerHTML = vidasCorazon[--vidasRival];
    } else{
        mensajes('Perdiste',`${personajeJugador} -<i class="fi fi-ss-heart" id="icon-mensaje"></i>`);
        document.getElementById("vida-jugador").innerHTML = vidasCorazon[--vidasJugador];
    }
    revisarVidas();
};
